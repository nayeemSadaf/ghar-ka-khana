...
namespace ghar_ka_khana
{
    public partial class LoginWithOTP : System.Web.UI.Page
    {
        ...
        protected void btnLgnOTPSbt_Click(object sender, EventArgs e)
        {
            if (captchacode.Text.ToLower().ToString() == Session["sessionCaptcha"].ToString())
            {
                ...
                if (read.HasRows)
                {
                    ...
                    if (read2.Read())
                    {
                       ...
                       // string LoginOTP = "555";
                        string LoginOTP = myRandomCode.ToString();

                        con.Open();
                        string qr = "update [user_info] set loginOTP='" + LoginOTP + "', loginOTPRequestTime='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where email_id ='" + email.Text.ToString() + "'";
                        ...
                        mail.Subject = "Hello test email from Anjali.";
                        ...
                        emailBody += "<p style='font-family:Arial,Helvetica,Verdana,sans-serif; font-size:14px;line-height:22px;color:rgb(41,41,41)'>OTP: " + LoginOTP + "</p>";
                        emailBody += "<p><a href='" + "http://localhost:1137/ConfirmLoginWithOTP.aspx"+ "'>Click here provide OTP for login.</a></p>";
